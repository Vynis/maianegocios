...
export class RolesListComponent implements OnInit, OnDestroy {
	 ...
	 */
	ngOnInit() {
		this.subheaderService.setTitle('Perfis');
	...
	deleteRole(_item: Role) {
		const _title = 'Perfil do Usuario';
		const _description = 'Tem certeza que deseja excluir o registro?';
		const _waitDesciption = 'Deletando o perfil...';
		const _deleteMessage = `Perfil deletado `;

		if (_item.id == 1 || _item.id == 4){
			const message = `Não é possível excluir perfil do tipo Administrador e Vendedor.`;
	 ...
	 */
	editRole(role: Role) {
		const _saveMessage = `Perfil salvo com sucesso.`;
